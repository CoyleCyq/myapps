<template>
  <div v-loading.fullscreen="listLoading" element-loading-background="rgba(0, 0, 0, 0.8)" class="app-container">
    ...
    >
      ...
      <el-table-column label="典雅" width="70" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.elegantValue }}</span>
        </template>
      </el-table-column>
      <el-table-column label="清新" width="70" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.freshValue }}</span>
        </template>
      </el-table-column>
      <el-table-column label="甜美" width="70" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.sweetValue }}</span>
        </template>
      </el-table-column>
      <el-table-column label="性感" width="70" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.sexyValue }}</span>
        </template>
      </el-table-column>
      <el-table-column label="帅气" width="70" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.handsomeValue }}</span>
        </template>
      </el-table-column>
      <el-table-column label="影之召唤" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.callOfShadow }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="1000px">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 100%; padding: 0 20px;">
        ...
        <el-form-item label="技能1">
          <el-input v-model="temp.skill1" />
        </el-form-item>
        <el-form-item label="技能2">
          <el-input v-model="temp.skill2" />
        </el-form-item>
        <el-form-item label="技能3">
          <el-input v-model="temp.skill3" />
        </el-form-item>
        <el-form-item label="影之召唤">
          <el-input v-model="temp.callOfShadow" />
        </el-form-item>
      </el-form>
...
<script>
import { fetchDesignerShadow, addDesignerShadow, updateDesignerShadow } from '@/api/designerShadow'
...
export default {
  ...
  data() {
    return {
      ...
      searchOptions: [{ label: '名称', key: 'name' }, { label: '影之召唤', key: 'callOfShadow' }],
      rules: {
  ...
  methods: {
    ...
    getList() {
      ...
      fetchDesignerShadow(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addDesignerShadow(this.temp).then((res) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateDesignerShadow(tempData).then(() => {
    ...
    handleDownload() {
      return debounce(() => {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['名称', '品质', '主属性', '属性值', '来源', '核心技能', '共鸣效果', '描述']
          const filterVal = ['name', 'level', 'mainAttr', 'attrvalue', 'source', 'skill', 'resonance', 'description']
