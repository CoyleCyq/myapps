<template>
  ...
  >
    ...
    <div class="filter-container">
      ...
      <el-button v-waves type="primary" @click="resetFilter">清空搜索</el-button>
      <el-button
        v-if="isAdmin"
        v-waves
    ...
    <el-table :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="操作" align="center" width="100" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button v-if="isAdmin" type="primary" size="mini" @click="handleUpdate(row)">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  data() {
    return {
      ...
      searchOptions: [
        { label: '名称', key: 'name' },
        { label: '标签', key: 'label' },
        { label: '设计师', key: 'author' },
        { label: '来源', key: 'source' }
      ],
      rules: {
        level: [{ required: true, message: '品阶是必选的', trigger: 'change' }],
        mainAttr: [
          { required: true, message: '主属性是必选的', trigger: 'change' }
        ]
      },
      temp: {
        // 新增数据
